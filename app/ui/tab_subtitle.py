...
def _run_gpu_sub(d, o, f, c, ac, ab, gpu, workers_val):
...
def _run_cpu_sub(d, o, f, c, ac, ab, workers_val):
    from app.pipeline.batch import run_batch_from_directory
    yield from run_batch_from_directory(
...
def build(gpu_selector):
    with gr.TabItem("GPU字幕烧录"):
        ...
        btn.click(fn=_run_gpu_sub, inputs=[input_dir, output_dir, out_fmt, crf, audio_codec, audio_br, gpu_selector, workers_gpu], outputs=log)
    ...
    with gr.TabItem("CPU字幕烧录"):
        ...
        log = gr.Textbox(label="处理日志", lines=20, autoscroll=True)
        audio_codec.change(fn=partial(_common_update_bitrates, default="128k"), inputs=audio_codec, outputs=audio_br)
        btn.click(fn=_run_cpu_sub, inputs=[input_dir, output_dir, out_fmt, crf, audio_codec, audio_br, workers_cpu], outputs=log)